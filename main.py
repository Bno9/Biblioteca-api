...
def deletar_livros_redis(livro_id: int, livro: Livro):
...
async def put_livros(id: int, livro: Livro, db: session = Depends(sessao_db), credentials: HTTPBasicCredentials = Depends(autenticar_usuario)):
   ...
   salvar_livros_redis(livro.id, livro)

   return {"message": "O livro foi atualizado com sucesso"}
    
@app.delete("/deletar/{id}")
async def delete_livros(id_livro: int, db: session = Depends(sessao_db), credentials: HTTPBasicCredentials = Depends(autenticar_usuario)):
