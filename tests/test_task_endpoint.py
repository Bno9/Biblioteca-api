...
def test_calcular_soma(mocker):
    ...
    response = client.post("/calcular/soma", params={"a": 4, "b": 5})

    assert response.status_code == 200
    assert response.json() == {"task_id": "fake-task-id", "status": "Tarefa de soma iniciada"}
...
def test_calcular_fatorial(mocker):
    ...
    assert response.status_code == 200
    assert response.json() == {"task_id": "fake-task-id", "status": "Tarefa de fatorial iniciada"}
    mock_redis_lpush.assert_called_once()
    mock_redis_ltrim.assert_called_once()